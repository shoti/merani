"""Pure token, artifact-count, and distribution calculations."""
...
ARTIFACT_BYTE_FIELDS = ("prompt_bytes", "manifest_bytes", "patch_bytes", "reviewer_report_bytes", "raw_response_bytes")


def empty_token_usage() -> dict[str, int]:
    return {**{field: 0 for field in TOKEN_FIELDS}, "total_input_tokens": 0, "total_tokens": 0}
...
def tokens_from_mapping(value: dict[str, Any]) -> dict[str, int]:
    aliases = {
        ...
        "cache_read_input_tokens": ("cache_read_input_tokens", "cacheReadInputTokens", "cache_read_tokens", "cached_input_tokens"),
    ...
    for field, keys in aliases.items():
        for key in keys:
            if key in value:
                ...
                break
    if "cached_input_tokens" in value and not any(key in value for key in ("cache_read_input_tokens", "cacheReadInputTokens", "cache_read_tokens")):
        result["input_tokens"] = max(0, result["input_tokens"] - result["cache_read_input_tokens"])
    result["total_input_tokens"] = sum(result[field] for field in ("input_tokens", "cache_creation_input_tokens", "cache_read_input_tokens"))
    result["total_tokens"] = result["total_input_tokens"] + result["output_tokens"]
    return result
...
def numeric_distribution(values: Iterable[float | int]) -> dict[str, float | int]:
    numbers = sorted(float(value) for value in values if not isinstance(value, bool) and isinstance(value, (int, float)) and math.isfinite(float(value)))
    if not numbers:
        return {"count": 0}
    return {
        "count": len(numbers), "min": round(numbers[0], 6),
        "p50": round(percentile(numbers, 0.5), 6), "p90": round(percentile(numbers, 0.9), 6),
        "max": round(numbers[-1], 6), "mean": round(sum(numbers) / len(numbers), 6),
    }
