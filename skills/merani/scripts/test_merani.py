...
from tests.unit.test_reflection import ReflectionTests
from tests.unit.test_campaign import CampaignSupportTests
...
class RunnerUnitTests(unittest.TestCase):
    def test_reflection_failure_is_advisory_and_redacts_exception_value(self) -> None:
        error = OSError("sk-synthetic-reflection-secret")
        output = io.StringIO()
        with (
            mock.patch.object(MM, "generate_run_reflection", side_effect=error),
            mock.patch.object(MM, "runtime_identity", return_value={}),
            redirect_stderr(output),
        ):
            self.assertIsNone(
                MM.refresh_run_reflection(
                    Path("/synthetic/run"), operation="fault-injection"
                )
            )
        self.assertIn("primary command outcome is unchanged", output.getvalue())
        self.assertNotIn("sk-synthetic", output.getvalue())
